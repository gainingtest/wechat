...
function fnCity(code){
    $.ajax({
        ...
        success:function(data){
            if(data.result){
                citys_data = data.object.baseAreaList;
                var area2 = new LArea();
                area2.init({
                    'trigger': '#demo2',
                    'valueTo': '#value2',
...
function fnProvince(){
    $.ajax({
        ...
        success:function(data){
            if(data.result){
                provs_data = data.object.baseAreaList;

                fnCity(provs_data[0].gId);
            }else{
                fnLayerBox(data.info);
            }
        }
...
window.LArea = (function() {
    ...
    MobileArea.prototype = {
                    ...
                    case 2:
                        if(_self.index==0){
                            $.ajax({
                                ...
                                success:function(data){
                                    if(data.result){
                                        childData = data.object.baseAreaList;
                                    }else{
                                        fnLayerBox(data.info);
                                    }
                                }
        ...
        check: function(e){
            if($('.hukou_city_select').val() === ''){
                $('.hukou_city_select').parents('li').find('.wrong_tips').text('请选择开户行所在地！').show();
                return false;
            }else{
                $('.hukou_city_select').parents('li').find('.wrong_tips').text('').hide();
                return true;
            }
        },
        close: function(e) {
